...
function UpdateModal( {show,handleClose,ele,getMov,
    titleInput,
    setTitleInput,
    imageInput,
    setImageInput,
    ...
    const update = async(id)=>{
        ...
        let updated = {id:ele.id, title:titleInput, release_date:ele.release_date ,poster_path:ele.poster_path,overview:ele.overview}
        
      await axios.put(`https://app-rawzi.herokuapp.com/update/${id}`,updated)
                  .then(()=>{
                    getMov();
                  }).catch(err=>{
                      console.log(err);
